...
class CompaniesControllerTest < ApplicationSystemTestCase
  ...
  test "Delete" do
    @new_company = companies(:updated_attribute_fixtures)
    companies_count = Company.count
    visit company_path(@new_company)
    page.has_link?(:href=>company_path(@new_company))
    find_all('.delete-company')[0].click
    page.driver.browser.switch_to.alert.accept
    assert_equal companies_count, Company.count
  end

  test 'Set city and state via zip code' do
    visit edit_company_path(@company)
    within("form#edit_company_#{@company.id}") do
      fill_in('company_zip_code', with: "93003", fill_options: {clear: :backspace})
      click_button 'Update Company'
      @company.reload
